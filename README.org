...
If session is connected to kernel keyring (i.e. ~PROTON_PASS_KEY_PROVIDER~ = ~keyring~) then [[https://protonpass.github.io/pass-cli/][documentation]] on use can be followed directly.
 
If you have no kernel keyring capabilities then ~PROTON_PASS_KEY_PROVIDER~ must be set to ~fs~ or ~env~ as per the [[https://protonpass.github.io/pass-cli/get-started/configuration/][documentation]]. Useful for containers but *not* secure.

For info on keyring access see [[*Keyring access][keyring access]].
